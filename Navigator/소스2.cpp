...
const street ways[] = {
	...
	{	28,	145	},{	30,	52	},{	31,	169	},// 3/29 3rd data was{3,169} error 고침
	{	14,	359	},{	29,	52	},{	27,	132	},
	{	28,	52	},{	34,	53	},{	30,	145	},
...
enum   p_state {
	...
	touched			// place가 처리 됐음
};
struct p_data {
	p_state p_stat;			// status
	int		come_from;		// 어느 지점에서 왔나, navi 출력시 필요
	int		pre_chain;		// front chain에서 앞, 자신을 가리키면 끝으로 취급
	int		post_chain;		// front chain에서 뒤, 자신을 가리키면 끝으로 취급
	int		dist_f_start;	// 시작점으로 부터 거리
};
p_data	plc[place_max]{};
p_data	chain_start;	// front chain의 시작을 가르킴
// plc[]의 data형식('p_data')을 취하지만 'post_chain'만 사용
// program을 끝내고 보니, 그냥 'int'형식도 무방함
//
// const int		place_max,	지점수 77
// const string		names[],	지점 이름
//
...
//
void plc_init() {	// 출발점과 도착지점이 입력된 후 뒤 'find_way()'를
	// 실행하기 전에 실행해야 할 routine
	...
	chain_start.post_chain = start_i;	// 출발점
	//
...
// 
// front chain의 시작점에서 갈 수 있는 점을 front chain에 삽입할 때 사용
//
// chain_ind		: front chain의 시작점, 다음점(...)으로 사용
//					  program을 끝내고 보니
//					  local 변수로 'int chain_ind = chain_start.post_chain;'로 초기화 가능
// neighbor_ind		: front chain에 넣을 점
// new_dist_f_start : 시작점 부터 chain start까지 거리 + neighbor까지 거리
void insert_front_chain(int chain_ind, int neighbor_ind, int new_dist_f_start) {
	int post_ind;
	while ((post_ind = plc[chain_ind].post_chain) != chain_ind) { // 3/29 changed
		if (new_dist_f_start <= plc[post_ind].dist_f_start) {
			// neighbor까지 거리가 chain의 post_ind와 ind까지 거리 사이이므로
			// insert neighbor btw. post_ind and ind
			plc[neighbor_ind].pre_chain = chain_ind;
			plc[neighbor_ind].post_chain = post_ind;
			plc[chain_ind].post_chain = neighbor_ind;
			plc[post_ind].pre_chain = neighbor_ind;
			return;
		}
		chain_ind = post_ind; // next in front chain
	}
	// front chain 끝 값 보다 큰 경우 끝에 삽입
	plc[chain_ind].post_chain = neighbor_ind;
	plc[neighbor_ind].post_chain = neighbor_ind;
	plc[neighbor_ind].pre_chain = chain_ind;	// 그러니까 이곳이 끝
	return;
}
...
void find_way() {
	...
	// program을 끝내고 보니 ind 대신 chain_start.post_chain을 써도 무방
	// 다만 이 곳에서는 chain_start를 <int>로 정의 함이 좋음
	int ind;	// 3/29 changed
	while ((ind = chain_start.post_chain) != end_i) {	// 3/29 changed
		...
		if (debug) cout << names[ind] << " is top, dist " << plc[ind].dist_f_start << endl;
		for (; ways_ind < ways_end; ways_ind++) {
			...
			if ((p_st = n_i_ptr->p_stat) != touched) {
				...
				if (p_st == not_touched) {
					...
					if (debug) cout << " new " << names[neighbor_ind] << " is neighbor " << new_dist_f_start << endl;
				}
				else {
					...
					if (new_dist_f_start < n_i_ptr->dist_f_start) {
						...
						// insert neighbor_ind to front chain
						insert_front_chain(ind, neighbor_ind, new_dist_f_start);
						if (debug) cout << " old " << names[neighbor_ind] << " is neighbor " << new_dist_f_start << endl;
					}
				}
			}
		}
		// 3/29 changed, ind 항이 처리 되었으므로
		// front chain에서 제거
		plc[ind].p_stat = touched;
		chain_start.post_chain = plc[ind].post_chain;
		if (debug) cout << " del " << names[ind] << endl;
	}
}

void disp_way() {
	//
	// 우선 p_data plc안에 come_from은 어디로부터 왔는지 표시 하므로
	// start_i로 부터 출력하려면 순서를 뒤 바꿔줘야 함
	// son은 father로 부터 father는 grand_father로 부터 온 것을
	// grand_father로 부터 father로 father로 부터 son으로
	// 
	...
	if (start_i == end_i) cout << "출발점과 종점이 같습니다." << endl;
		...
		while (index != end_i) {
			...
			// 3/29 changed
			// index++; error, next line is right
			index = plc[index].come_from;
		}
